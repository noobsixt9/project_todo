...
Public Class add_task
    ...
    Public Sub savebtn_Click_1(sender As Object, e As EventArgs) Handles savebtn.Click
        ...
        Try
            ...
            Else
                If dashboard_form.IsNewTask = True Then
                    ...
                    End If
                ElseIf dashboard_form.IsNewTask = False Then
                    Try
                        Dim sql As String = "update todolist set title = @title, description = @description, duedate = @duedate where id = @taskid and userid = @userid"
                        Dim command As New SqlCommand(sql, connection)
                        command.Parameters.AddWithValue("@userid", userid)
                        command.Parameters.AddWithValue("@title", txttitle.Text)
                        command.Parameters.AddWithValue("@description", textdescp.Text)
                        command.Parameters.AddWithValue("@duedate", duedate.Value.Date)
                        command.Parameters.AddWithValue("@taskid", tasks_form.taskid)
                        Dim rowsaffected As Integer = Convert.ToInt32(command.ExecuteNonQuery())
                        If rowsaffected > 0 Then
                            ...
                            tasks_form.LoadTasksFromDatabase(login_form.LoggedInUser)
                        Else
                            MessageBox.Show($"Task failed to update. ", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        End If
